...
# PASSWORD GATE
# ============================================================
# The password is read from Streamlit's secrets manager (set via the Streamlit
# Cloud dashboard). It's NOT stored in this file or in the GitHub repo.
def check_password():
    ...
    # Show login screen in SPORTFIVE style
    st.markdown(f"""
    <style>
    .stApp {{ background: {BRAND['white']}; font-family: {BRAND['font_family']}; }}
    header[data-testid="stHeader"] {{ display: none; }}
    .sf-login-box {{
        max-width: 480px; margin: 80px auto 0 auto;
        padding: 48px; border: 2px solid {BRAND['black']};
    }}
    .sf-login-title {{
        font-size: 32px; font-weight: 900; text-transform: uppercase;
        letter-spacing: -1px; margin: 0 0 8px 0; color: {BRAND['black']};
    }}
    .sf-login-sub {{
        font-size: 12px; letter-spacing: 2px; text-transform: uppercase;
        color: {BRAND['text_muted']}; margin-bottom: 28px; font-weight: 600;
    }}
    </style>
    <div class="sf-login-box">
        <div class="sf-login-title">{BRAND['company_name']}</div>
        <div class="sf-login-sub">{BRAND['tool_name']}</div>
...
if not check_password():
    st.stop()
# ============================================================
# Everything below here only runs if password is correct
# ============================================================
