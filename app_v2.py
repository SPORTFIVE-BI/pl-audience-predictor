...
def render_prediction_card(title, p10, p50, p90, prominent=False):
    ...
    <div class="{card_cls}">
        <div style="display: flex; align-items: flex-start; justify-content: space-between; gap: 24px;">
            <div style="flex: 1;">
                <div class="sf-pred-label">{title}</div>
                <div class="sf-pred-headline">{format_audience(p50)}</div>
                <div class="sf-pred-range">
                    Reasonable Range: <strong>{format_audience(p10)} – {format_audience(p90)}</strong>
                    <span style="margin-left: 16px; opacity: 0.7;">±{range_pct/2:.0f}%</span>
                </div>
            </div>
            <div style="text-align: right; padding-top: 8px;">
                {conf_chip_html(level)}
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if home == away:
        ...
        if st.button("Predict", type="primary"):
            ...
            for region in ["uk","europe_excl_uk","asia_pacific","latin_america",
                ...
                rows += f"""
                <div class="sf-region-row">
                    <div>
                        <div class="sf-region-name">{REGION_LABELS[region]}</div>
                        <div class="sf-region-range">{format_audience(v['p10'])} – {format_audience(v['p90'])}</div>
                    </div>
                    <div style="text-align: right;">
                        <div class="sf-region-value">{format_audience(v['p50'])}</div>
                        <div class="sf-region-level">{level}</div>
                    </div>
                </div>
                """
