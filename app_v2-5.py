...
# Confidence thresholds. These compare against HALF-WIDTH — i.e. the ±X% number
# shown to the user. Calibrated against the 2025/26 backtest:
#   ±35% or tighter → High   (~14% of fixtures, median actual error 23%)
#   ±35%–60% → Medium        (~45% of fixtures, median actual error 17%)
#   ±60%+ → Low              (~40% of fixtures, median actual error 43%)
CONF_HIGH_MAX_PCT = 35
CONF_MED_MAX_PCT = 60
...
def assess_confidence(p10, p50, p90):
    """Return (level, half_width_pct). Both the chip and the displayed ±X%
    are based on the half-width of the P10-P90 band."""
    if p50 <= 0:
        return ("Low", 0)
    # Half-width = the ±X% the user sees
    half_width = (p90 - p10) / p50 / 2 * 100
    if half_width <= CONF_HIGH_MAX_PCT:
        return ("High", half_width)
    if half_width <= CONF_MED_MAX_PCT:
        return ("Medium", half_width)
    return ("Low", half_width)
...
def render_prediction_card(title, p10, p50, p90, prominent=False):
    level, half_width = assess_confidence(p10, p50, p90)
    ...
    html = (
        ...
        f'<span style="margin-left: 16px; opacity: 0.7;">±{half_width:.0f}%</span>'
        f'</div>'
        f'</div>'
...
# Empirically-calibrated basket P10/P90 ratios from the 2025/26 backtest.
# Each value is (P10 ratio, P90 ratio): if you sum N fixture P50s and call that
# the basket P50, then the realistic 80% range is [P50 * P10_ratio, P50 * P90_ratio].
BASKET_SPREAD = {
    1:  (0.51, 1.69),
    2:  (0.65, 1.42),
    3:  (0.71, 1.30),
...
def basket_range(p50_sum, n):
    """Return (P10, P90) for a basket of size n, using empirical calibration."""
    sizes = sorted(BASKET_SPREAD.keys())
    if n <= sizes[0]:
        ...
        for i in range(len(sizes) - 1):
            if sizes[i] <= n <= sizes[i + 1]:
                ...
                lo_a, hi_a = BASKET_SPREAD[sizes[i]]
                lo_b, hi_b = BASKET_SPREAD[sizes[i + 1]]
                lo = lo_a + t * (lo_b - lo_a)
                hi = hi_a + t * (hi_b - hi_a)
                break
...
def show_basket_results(df_pred, n_fixtures):
    ...
    if totals is not None:
        ...
        # Use the EMPIRICAL basket spread, not the naive sum of P10s/P90s.
        # The naive sum assumes all errors push the same direction at once —
        # they don't, so it produces ranges that are 3-4x too wide.
        p10_calibrated, p90_calibrated = basket_range(totals["p50"], n_fixtures)
        render_prediction_card(f"Package of {n_fixtures} fixtures",
                                p10_calibrated, totals["p50"], p90_calibrated,
                                prominent=True)
    ...
    if totals is not None:
        with st.expander("Regional Breakdown For Package Total"):
            ...
            # Apply the same basket calibration per region
            calib = region_df.apply(
                lambda r: basket_range(r["p50"], n_fixtures), axis=1, result_type="expand")
            region_df["p10_cal"] = calib[0]
            region_df["p90_cal"] = calib[1]
            region_df["P50"] = region_df["p50"].apply(format_audience)
            region_df["Range"] = region_df.apply(
                lambda r: f"{format_audience(r['p10_cal'])} – {format_audience(r['p90_cal'])}", axis=1)
            region_df["Confidence"] = region_df.apply(
                lambda r: assess_confidence(r["p10_cal"], r["p50"], r["p90_cal"])[0], axis=1)
...
<div class="sf-footer">
    ...
    Headline figure is the most likely audience. The range shows where roughly 4 out of 5 actual outcomes are expected to fall.
</div>
""", unsafe_allow_html=True)
